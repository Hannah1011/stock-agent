...
| **Guardrails** | 구현 | `utils/guardrails.py`: 범위 밖 질문 차단, 투자 권유 문구 치환, 면책 문구 추가. 에이전트 출력과 실행 계획을 Pydantic 모델로 구조화하고, 모델 생성 시 필수 필드·타입과 일부 값 범위를 검증합니다. |
...
| **Exception Handling** | 구현 | 주요 에이전트와 워크플로우에서 API 오류·데이터 부족을 처리합니다. 뉴스 키워드 스코어링, 차트·포트폴리오 기본 해설, 뉴스·종목 목록 데이터 소스 폴백을 지원합니다. |
...
앱은 항상 고정 주소 `http://localhost:8501`을 사용합니다. 이미 앱이 실행 중인 상태에서 `bash run.sh`를 다시 입력하면 새 서버를 중복 실행하지 않고 기존 접속 주소를 안내합니다.

브라우저 창을 닫아도 실행 터미널이 열려 있으면 같은 주소로 다시 접속할 수 있습니다. 기존 앱이 멈춘 채 `8501` 포트만 점유하고 있으면 `run.sh`가 해당 프로젝트의 프로세스만 종료하고 같은 주소에서 다시 시작합니다. 앱을 완전히 종료할 때는 실행 중인 터미널에서 `Ctrl+C`를 누릅니다.
