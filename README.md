...
- 특정 종목의 최신 뉴스 수집과 관련도·파급력 평가
- 이동평균, RSI, 볼린저밴드, 거래량 기반 차트 분석
...
| **RAG** | 구현 | `rag/retriever.py`: 경제 용어 코퍼스에 대해 BM25 + ChromaDB Dense Search를 수행하고 RRF로 결과를 결합합니다. 과거 뉴스 DB 검색은 구현되어 있지 않습니다. |
...
| **HITL** | 구현 | 종목명 매칭 신뢰도가 낮으면 실행을 멈추고 사용자에게 종목 후보를 선택받은 뒤 워크플로우를 재개합니다. |
...
| **Learning and Adaptation** | 부분 구현 | RAG 사전에 없는 용어를 Wikipedia + Claude로 설명해 `dynamic_terms.json`에 저장하고 이후 검색에 재사용합니다. 모델 학습이나 자동 프롬프트 개선은 하지 않습니다. |
| **Evaluation and Monitoring** | 부분 구현 | 실행 모니터링은 구현되어 있지만 정답 데이터셋 기반 품질 평가, 회귀 평가, LLM judge는 아직 없습니다. |
| **Reflection** | 미구현 | 결과를 비평한 뒤 스스로 다시 작성하는 별도 Reflection Agent 또는 반복 루프는 없습니다. |
| **MCP** | 미구현 | 외부 도구는 Python 모듈과 HTTP API로 직접 연결하며 MCP 서버를 사용하지 않습니다. |
| **A2A** | 미구현 | 에이전트는 LangGraph 내부 상태로 협업하며 별도의 Agent-to-Agent 프로토콜은 사용하지 않습니다. |
| **Goal Setting** | 미구현 | 장기 목표를 생성하고 진행률을 추적하는 목표 관리 계층은 없습니다. |
...
필수입니다. Orchestrator와 모든 텍스트 생성 에이전트가 사용합니다.
...
선택 사항입니다. 키가 없거나 NewsAPI 호출이 실패하면 Google News RSS로 자동 폴백합니다.
...
API 키가 들어 있는 `.env`는 Git에 커밋하지 않습니다.
...
기본값인 `STOCK_CATALOG_PROVIDER=local`에서는 KRX를 직접 호출하지 않습니다. 앱은 키워드 맵과 `portfolio.json`으로 종목 목록을 만들고 Yahoo Finance에서 섹터를 보완합니다.

`STOCK_CATALOG_PROVIDER=pykrx`를 선택하면 KRX 조회를 먼저 시도하지만, KRX가 비로그인 요청에 `LOGOUT`을 반환하는 환경에서는 로컬+Yahoo 방식으로 자동 전환됩니다.

`tools/.cache/krx_YYYYMMDD.json`의 날짜는 파일 생성일이 아니라 기준 거래일입니다. 예를 들어 2026년 6월 14일은 일요일이므로 직전 거래일인 2026년 6월 12일이 파일명에 사용됩니다.
